/**
     ...
     * `processEditInput` appends one NDJSON line to `/tmp/pi-edit-guard-<pid>.log`
     * with sha256 + length + preview of every oldText/newText/fileContent, plus
     * the cascade verdict and autofix outcome. No full content is ever logged.
     */
    ...
    async function processEditInput(
      ...
      debug.edits = evaluations.map((evaluation, i) => buildDebugEdit(edits[i], evaluation, autofixOutcomes.get(i)));
