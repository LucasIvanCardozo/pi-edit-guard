    ...
    export function run(): void {
      ...
      section('autofix: tryAutofix uniform +2 spaces shift (single-line)');
      {
        ...
        if (fix.ok) {
          ...
          assertEq(fix.result.correctedNewText, '    return 2;', 'correctedNewText has shifted leading spaces');
      ...
      section('autofix: tryAutofix uniform -2 spaces shift (multi-line)');
      {
        ...
        if (fix.ok) {
          ...
          assertEq(fix.result.correctedNewText, '  return 2;\n}', 'newText leading spaces shifted by -2 uniformly');
      ...
      section('autofix: tryAutofix declines for empty newText (delete case)');
      {
        ...
        if (!fix.ok) {
          assertEq(fix.decline.reason, 'zero-delta', 'specific reason reported (treated as zero-delta)');
        }
      }
